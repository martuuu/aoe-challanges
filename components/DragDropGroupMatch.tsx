...
export function DragDropGroupMatch({ allPlayers, onCreateMatch }: DragDropGroupMatchProps) {
  ...
  const availablePlayers = allPlayers.filter((player) => !team1.includes(player) && !team2.includes(player))
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-1">
      ...
      <div>
        <Label className="text-sm sm:text-base font-semibold mb-2 sm:mb-3 block text-green-700">Jugadores Disponibles</Label>
        <div
        ...
        >
          {availablePlayers.length === 0 ? (
            <span className="text-gray-400 text-xs sm:text-sm">Todos los jugadores están asignados a equipos</span>
          ) : (
        ...
        <p className="text-xs text-green-600 mt-1 sm:mt-2">
          ...
          <span className="sm:hidden">Usa los botones →1 o →2 para asignar jugadores a equipos</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        ...
        <div>
          ...
          >
            <div className="flex flex-wrap gap-1 sm:gap-2">
              {team1.map((player) => (
                <Badge key={player} className="flex items-center gap-1 bg-[#819067] text-white text-xs sm:text-sm px-2 sm:px-3 py-1">
                  {player}
                  <X
        ...
        <div>
          ...
          >
            <div className="flex flex-wrap gap-1 sm:gap-2">
              {team2.map((player) => (
                <Badge key={player} className="flex items-center gap-1 bg-[#1B3C53] text-white text-xs sm:text-sm px-2 sm:px-3 py-1">
                  {player}
                  <X
      ...
      {team1.length > 0 && team2.length > 0 && (
        <div>
          <Label className="text-sm sm:text-base font-semibold mb-2 sm:mb-3 block text-green-700">Equipo Ganador</Label>
